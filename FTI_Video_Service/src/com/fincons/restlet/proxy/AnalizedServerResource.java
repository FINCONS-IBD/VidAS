...
public class AnalizedServerResource extends ServerResource {

	final static Logger logger = Logger.getLogger(AnalizedServerResource.class);
	
	@Post
	public Representation analizedProxy(String parameters) {
		...
		try {
			...
			String urldownloadEncryptedFile= PropertiesHelper.getProps().getProperty(Constants.PATH_FILE)+"/"+nameFile.split("/")[1];
			String pathFile = AnalizedServerResource.class.getClassLoader().getResource(PropertiesHelper.getProps().getProperty(Constants.PATH_FILE))
			...
			String pathFileDecrypted= AnalizedServerResource.class.getClassLoader().getResource(PropertiesHelper.getProps().getProperty(Constants.PATH_FILE)).getPath()+nameFileDecrypt;
...
//					jsonResponse.put("url", PropertiesHelper.getProps().getProperty(Constants.PATH_FILE)+"/"+nameFileDecrypt);
					jsonResponse.put("code", 200);
					jsonResponse.put("url", urldownloadEncryptedFile);
//				}
			} catch (JSONException e) {
